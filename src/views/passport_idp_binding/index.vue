  ...
  <div class="passport_idp_binding-page">
    ...
    <el-card class="passport_idp_binding-page__search" shadow="never">
      ...
      <QueryForm ref="queryFormRef" v-model="baseQueryForm" @search="handleSearch">
        ...
        <el-form-item label="IDP侧用户授权标识">
          <el-input v-model="queryForm.idpSubject" placeholder="请输入IDP侧用户授权标识" clearable style="width: 200px" />
        </el-form-item>
        <el-form-item label="IDP侧企业ID">
          <el-input v-model="queryForm.corpId" placeholder="请输入IDP侧企业ID" clearable style="width: 200px" />
        </el-form-item>
        <el-form-item label="IDP侧用户ID">
          <el-input v-model="queryForm.idpUserId" placeholder="请输入IDP侧用户ID" clearable style="width: 200px" />
        </el-form-item>
        <el-form-item label="IDP侧的应用ID">
          <el-input v-model="queryForm.idpApplicationCode" placeholder="请输入IDP侧的应用ID" clearable style="width: 200px" />
        </el-form-item>
        <el-form-item label="接入形态">
          <el-input v-model="queryForm.bindMode" placeholder="请输入接入形态" clearable style="width: 200px" />
        </el-form-item>
    ...
    <SortableTable :data="tableData" border stripe style="width: 100%" :enable-multi-sort="true" @sort-change="handleSortChange">
      ...
      <el-table-column prop="idpSubject" label="IDP侧用户授权标识" width="180" />
      <el-table-column prop="corpId" label="IDP侧企业ID" width="180" />
      <el-table-column prop="idpUserId" label="IDP侧用户ID" width="180" />
      <el-table-column prop="idpApplicationCode" label="IDP侧的应用ID" width="180" />
      <el-table-column prop="bindMode" label="接入形态" width="180" />
      <TableColumn prop="createTime" label="创建时间" width="180" :sortable="true" />
      <TableColumn prop="updateTime" label="更新时间" width="180" :sortable="true" />
      <el-table-column label="操作" fixed="right" width="160">
        <template #default="{ row }">
          <el-button type="primary" link size="small" @click="handleView(row)">明细</el-button>
          <el-button type="danger" v-permission="'passport_idp_binding:delete'" link size="small" @click="handleDelete(row)">删除</el-button>
        </template>
        <template #header>
    ...
    <!-- 明细抽屉 -->
    <el-drawer v-model="detailDialogVisible" title="账号与外部身份源绑定表明细" direction="rtl" size="720px" destroy-on-close>
      <el-descriptions :column="1" border label-width="220px" class="passport-idp-binding-detail">
        ...
        <el-descriptions-item label="IDP侧用户授权标识">{{ currentRow?.idpSubject }}</el-descriptions-item>
        <el-descriptions-item label="IDP侧企业ID">{{ currentRow?.corpId }}</el-descriptions-item>
        <el-descriptions-item label="IDP侧用户ID">{{ currentRow?.idpUserId }}</el-descriptions-item>
        <el-descriptions-item label="IDP侧的应用ID">{{ currentRow?.idpApplicationCode }}</el-descriptions-item>
        <el-descriptions-item label="接入形态">{{ currentRow?.bindMode }}</el-descriptions-item>
        <el-descriptions-item label="IdP 返回的原始用户信息快照">
...
const handleDelete = (row: PassportIdpBinding) => {
  ElMessageBox.confirm(`确认删除账号与外部身份源绑定表「${row.id}」吗？`, '提示', {
    type: 'warning',
  })
    .then(async () => {
      try {
